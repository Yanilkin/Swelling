...
def f(c_all,t):
	...
	dc[0,0] = G*prbias[0] - ((k2vs+k2vd+k2v+k2vp[0,0]*c[0,0]+k2mp[0,0]*cm)*Dv + k2ip_c[0,0]*Dici + k2mp_c[0,0]*Dmcm)*c[0,0] + k2ip_c[0,1]*Dici*c[0,1] + k2vp_c[0,0]*Dv*cveq[0,1]*c[0,1] + k2mp_c[0,0]*(Dm+Dv)*cmeq[1,0]*c[1,0] + Dv*np.sum(k2vp[:,:max_n-1]*cveq[:,1:]*c[:,1:])
...
if __name__ == "__main__":
	...
	print "total impurity concentration\n", cm[-1]+np.sum(clusters[-1,max_n:]*np.repeat(np.arange(1,max_c,1),max_n))
	#print "total swelling\n," np.sum(results[-1]*sizes/Va)
